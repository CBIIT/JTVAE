...
#JI     self.stop_loss = nn.BCEWithLogitsLoss(size_average=False)
    ...
    def forward(self, mol_batch, x_tree_vecs):
        ...
        for t in range(max_iter):
            ...
            #Message passing
            cur_h_nei = torch.stack(cur_h_nei, dim=0).view(-1,MAX_NB,self.hidden_size) #BREAKING
            new_h = GRU(cur_x, cur_h_nei, self.W_z, self.W_r, self.U_r, self.W_h)

            #Node Aggregate
            cur_o_nei = torch.stack(cur_o_nei, dim=0).view(-1,MAX_NB,self.hidden_size) #BREAKING
